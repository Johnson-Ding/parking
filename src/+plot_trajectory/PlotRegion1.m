function PlotRegion1(obstacleList, state, input, num, VEHICLE, CONFIG, imgPath, figNum, figTitle, T)
%PLOTMOVINGREGIONS 显示车辆运动过程中扫过的区域
%   obstacleList是障碍物的位置列表，是m*3的矩阵，第一列到第三列依次为横坐标、纵坐标、偏航角
%   path0是车辆运动路径，是n*5的矩阵，第一列到第五列依次为横坐标、纵坐标、偏航角、速度、后轴中心曲率
%   input是n*2的矩阵，第一列是加速度a，第二列是曲率变化率sigma
%   num是两个配置点之间插值点的个数
